...
authController.edit = (req, res) => {
  ...
  try {
    ...
    if (decoded._id == id || decoded.isAdmin) {
      Mongo.MongoClient.connect(mongoUrl, (err, client) => {
        ...
        db.collection("users")
          ...
          .then(() => {
            console.log("termino bien la edicion");
            db.collection("users")
              .findOne({ _id: id })
              .then(userEdit => {
                console.log("recogiendo el userEdit de DB");
                if (err) throw err;
                console.log("edito bien");
                res.send(userEdit);
                client.close();
              });
          })
    ...
    res
      .status(400)
      .send(
        "El token no es válido o no existe o fallo en conexionBD, " + error
      );
  }
};
